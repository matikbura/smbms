...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/provider/deleteProviderById",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/queryOne?proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/preModify?proid="+ obj.attr("proid");
	});
